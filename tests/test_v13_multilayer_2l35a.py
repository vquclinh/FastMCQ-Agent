...
def _load(script):
    spec = importlib.util.spec_from_file_location(f"v13_{script}", _ROOT / "scripts" / f"{script}.py")
...
def test_no_qid_hardcoding_v13():
    ...
    for name in ("build_v13_multilayer_plan", "run_v13_multilayer_verifier",
        ...
        assert not re.search(r"\btest_\d{4}\b", (_ROOT / "scripts" / f"{name}.py").read_text()), name
