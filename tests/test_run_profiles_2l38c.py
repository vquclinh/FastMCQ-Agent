...
def _fi():
    spec = importlib.util.spec_from_file_location("fi_prof", _ROOT / "scripts" / "final_infer.py")
...
def test_wrappers_reference_profiles():
    text = {w: (_ROOT / "scripts" / w).read_text() for w in _WRAPPERS}
...
def test_no_qid_or_answer_hardcoding():
    assert not re.search(r"\btest_\d{4}\b", (_ROOT / "configs" / "run_profiles.json").read_text())
    for w in _WRAPPERS:
        src = (_ROOT / "scripts" / w).read_text()
