...
def test_default_mode_is_frozen_csv():
    d = tempfile.mkdtemp(); out = f"{d}/pred.csv"
    rc, txt = _run_short(out)
    assert "mode: frozen_csv" in txt


def test_default_source_is_frozen_best():
    d = tempfile.mkdtemp(); out = f"{d}/pred.csv"
    rc, txt = _run_short(out)
    assert f"source: {_BEST}" in txt


def test_output_md5_matches_winning_csv():
    d = tempfile.mkdtemp(); out = f"{d}/pred.csv"
    _run_short(out)
...
def test_docs_contain_short_btc_command():
    short = "scripts/final_infer.py --input public-test_1780368312.json --output pred.csv"
    fr = (_ROOT / "FINAL_RUN.md").read_text()
    assert short in fr
    # short command appears near the very top (first ~25 lines)
    assert any(short in ln for ln in fr.splitlines()[:25])
