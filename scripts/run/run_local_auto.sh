#!/usr/bin/env bash
# Full local selective system (Base+V12B+V13).
# Usage: bash scripts/run_local_auto.sh <input_test_file> [extra final_infer flags...]
# Profile: local_selective_auto (see configs/profiles/run_profiles.json).
set -euo pipefail

INPUT="${1:?usage: bash scripts/run_local_auto.sh <input_test_file> [extra flags...]}"
shift || true

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
PY="$ROOT/.venv/bin/python"; [ -x "$PY" ] || PY="python"
TS="$(date +%Y%m%d_%H%M%S)"
RUN_DIR="$ROOT/scratch/runs/local_auto_$TS"
