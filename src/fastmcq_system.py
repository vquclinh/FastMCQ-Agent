...
def run_fastmcq_system(samples, output_csv, config: FastMCQSystemConfig) -> FastMCQSystemReport:
    ...
    base = predict_base_answers(
        samples, model=config.model, execute_api=config.execute_api,
    ...
    # 6) V13 multi-layer (optional; disabled by default). Deterministic programmatic path may run
    # offline; model-dependent layers are skipped_no_api without --execute-api.
    ...
    if config.enable_v13:
        ...
        if not config.execute_api:
            warnings.append("V13 enabled: model layers skipped_no_api; deterministic programmatic "
                            "path may apply")
