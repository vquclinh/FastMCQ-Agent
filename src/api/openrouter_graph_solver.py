...
from src.utils.labels import index_to_label, is_valid_label, labels_for
from src.api.openrouter_client import OpenRouterClient
from src.api.openrouter_prompts import build_messages, repair_messages
from src.evidence.passage_compressor import compress_passage
...
from src.utils.structured_answer import parse_structured_answer, response_format_schema
...
class OpenRouterGraphSolver(BaseSolver):
    ...
    def _evidence_node(self, s):
        ...
        if self.cfg.evidence_reranker_enabled:
            from src.evidence.evidence_reranker import rerank_evidence_for_sample
    ...
    def _calculation_node(self, s) -> bool:
        ...
        if not self.cfg.calc_enabled or s["route"] not in ("calculation", "ambiguous"):
            return False
        from src.solvers.calculation_solver import solve_calculation_sample
        sample = s["_sample"]
    ...
    def _verifier_node(self, s):
        ...
        from src.selector.mcq_verifier import (build_verifier_messages, parse_verification,
    ...
    def _adaptive_node(self, s):
        ...
        if not self.cfg.adaptive_reasoning_enabled or "adaptive" in s:
            return
        try:
            from src.layers.adaptive_orchestrator import AdaptiveConfig, AdaptiveOrchestrator
